...
include("$config[template_path]/admin_top.html");
	...
	if ($action == "update_pic")
		...
		if ($admin_privs == "yes")
			{
			$sql = "UPDATE " . $config[table_prefix] . "listingsImages SET caption = $sql_caption, description = $sql_description, rank = $sql_rank WHERE ((listing_id = $sql_edit) AND (file_name = $sql_pic))";
			}
		else
			{
			$sql = "UPDATE " . $config[table_prefix] . "listingsImages SET caption = $sql_caption, description = $sql_description, rank = $sql_rank WHERE ((listing_id = $sql_edit) AND (file_name = $sql_pic) AND (user_id = $userID))";
			}
		$recordSet = $conn->Execute($sql);
			if ($recordSet === false) log_error($sql);
	...
	if ($action == "delete_pic")
		...
		if ($admin_privs == "yes")
			{
			$sql = "SELECT file_name, thumb_file_name FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = $sql_edit) AND (ID = $pic_id))";
			}
		else
			{
			$sql = "SELECT file_name, thumb_file_name FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = $edit) AND (ID = $pic_id) AND (user_id = $userID))";
			}
		$recordSet = $conn->Execute($sql);
			if ($recordSet === false) log_error($sql);
		...
		if ($admin_privs == "yes")
			{
			$sql = "DELETE FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = $sql_edit) AND (file_name = '$file_name'))";
			}
		else
			{
			$sql = "DELETE FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = $sql_edit) AND (file_name = '$file_name') AND (user_id = '$userID'))";
			}
		$recordSet = $conn->Execute($sql);
			if ($recordSet === false) log_error($sql);
	...
	if ($admin_privs == "yes")
		{
		$sql = "SELECT ID, caption, file_name, thumb_file_name, description, rank FROM " . $config[table_prefix] . "listingsImages WHERE (listing_id = '$edit') ORDER BY rank";
		}
	else
		{
		$sql = "SELECT ID, caption, file_name, thumb_file_name, description, rank FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = '$edit') AND (user_id = '$userID')) ORDER BY rank";
		}
	$recordSet = $conn->Execute($sql);
	if ($recordSet === false) log_error($sql);
