<?php
	...
	?>
		...
		<tr><td colspan="2" class="row_main"><h3><?php echo $lang[create_new_user] ?></h3></td></tr>
		<tr>
			<td align="right" class="row_main"><b><?php echo $lang[user_name] ?> <span class="required">*</span></b></td>
			<td align="left" class="row_main"> <input type="text" name="edit_user_name"></td>
		</tr>
		<tr>
			<td align="right" class="row_main"><b><?php echo $lang[user_password] ?> <span class="required">*</span></b></b></td>
			<td align="left" class="row_main"> <input type="password" name="edit_user_pass"></td>
		</tr>
		
		<tr>
			<td align="right" class="row_main"><b><?php echo $lang[user_password] ?></b> (<?php echo $lang[again] ?>)<b><span class="required">*</span></b> </td>
			<td align="left" class="row_main"> <input type="password" name="edit_user_pass2"></td>
		</tr>
		
		<tr>
			<td align="right" class="row_main"><b><?php echo $lang[user_email] ?></b> <b><span class="required">*</span></b></td>
			<td align="left" class="row_main"> <input type="text" name="user_email"></td>
		</tr>
	<?php
			// is the user active?
		echo "<tr><td align=right><b>$lang[user_editor_active]: </b></td>";
		echo "<td align=left><select name=\"edit_active\" size=\"1\"><option value=\"no\">no<option value=\"yes\">yes</select></td></tr>";	
			
		// is the user an agent?
		//	echo "<tr><td align=right><b>$lang[user_editor_isAgent]: </b></td>";
		//	echo "<td align=left><select name=\"edit_isAgent\" size=\"1\"><option value=\"no\">no<option value=\"yes\">yes</select></td></tr>";
		...
		$sql = "SELECT field_type, field_name, field_caption, default_text, field_elements, rank, required from " . $config[table_prefix]. "$formDB ORDER BY rank, field_caption";
	...
	else
	{
		...
		else
		{
			...
			$pass_the_form = "No";
			$fail_message = $lang[required_fields_not_filled];
			...
			if ($pass_the_form == "Yes")
			{
				...
				$sql = "INSERT INTO " . $config[table_prefix] . "UserDB (user_name, user_password, emailAddress, creation_date, last_modified) VALUES ($sql_user_name, $random_number, $sql_user_email,".$conn->DBDate(time()).",".$conn->DBTimeStamp(time()).")";
				$recordSet = $conn->Execute($sql);
				if ($recordSet === false)
				{
					log_error($sql);
