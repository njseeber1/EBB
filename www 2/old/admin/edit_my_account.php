	...
	if ($action == "update_user")
	{
		...
		else
		{
			...
			if ($pass_the_form == "Yes")
			{
				...
				$sql = "UPDATE  " . $config[table_prefix] . "UserDB SET emailAddress = $user_email, user_password = $md5_user_pass, last_modified = ".$conn->DBTimeStamp(time())." WHERE ID = '$userID'";
	...
	else
	{
		...
		<tr>
			<td width="<?php echo $style[image_column_width] ?>" valign="top" align="center" class="row_main">
				...
				<br>
				<hr width="75%">
				<a href="edit_user_images.php?edit=<?php echo $userID ?>"><?php echo $lang[edit_images] ?></a><br><br>
				<?php

				$sql = "SELECT caption, file_name, thumb_file_name FROM " . $config[table_prefix] . "userImages WHERE user_id = '$userID'";
				$recordSet = $conn->Execute($sql);
		...
		$sql = "SELECT * FROM " . $config[table_prefix] . "UserDB WHERE ID = $userID";
		...
		if ($edit_isAgent == "yes")
		{
			$sql = "SELECT f.field_name, db.field_value, f.field_type, f.rank, f.field_caption, f.default_text, f.required, f.field_elements FROM " . $config[table_prefix] . "agentFormElements f left join " . $config[table_prefix] . "UserDBElements db on db.field_name = f.field_name and db.user_id = '$userID' ORDER BY f.rank";
		}
		else
		{
			$sql = "SELECT f.field_name, db.field_value, f.field_type, f.rank, f.field_caption, f.default_text, f.required, f.field_elements FROM " . $config[table_prefix] . "memberFormElements f left join " . $config[table_prefix] . "UserDBElements db on db.field_name = f.field_name and db.user_id = '$userID' ORDER BY f.rank";
		}
