...
if ($action == "updateUserFormElement")
	...
	$sql = "UPDATE " . $config[table_prefix] . "" . $formDB .  "  SET field_type = $sql_field_type, ";
...
if ($action == "DeleteUserFormElement")
	{
	$sql = "DELETE FROM " . $config[table_prefix] . "" . $formDB . " where ID = $id";
	$recordSet = $conn->Execute($sql);
	if ($recordSet === false) log_error($sql);
...
if ($action == "makeNewElement")
	...
	$sql = "INSERT INTO " . $config[table_prefix] . "" . $formDB . " ";
...
?>
	...
	<input type="hidden" name="edit_isAgent" value="<?php echo $edit_isAgent; ?>">
	<tr>
	...
	$sql = "SELECT id, field_type, field_name, field_caption, default_text, field_elements, rank, required FROM " . $config[table_prefix] . "" . $formDB . " ORDER BY rank, field_name";
	$recordSet = $conn->Execute($sql);
	if ($recordSet === false) log_error($sql);
	$count = 0;
	...
	while (!$recordSet->EOF)
		...
		<tr>
			...
			<td class="row_main" align="left"><input type="submit" name="Submit" value="<? echo $lang[update_button] ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="<? echo $PHP_SELF ?>?edit_isAgent=<?php print $edit_isAgent; ?>&action=DeleteUserFormElement&id=<? print $id ?>" onClick="return confirmDelete()"><? echo $lang[delete] ?></a></td>
		</tr></form>
