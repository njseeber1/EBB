 ...
 *    - Save As Draft
 *
 * $Id: compose.php,v 1.225.2.18 2002/11/20 06:49:39 jangliss Exp $
 */
...
elseif (isset($sigappend)) {
    ...
    // Modification for File Manager Plugin:
    // when sending attachments, we don't want
    // to lose the attachment(s) the user has
    // selected from the File Manager interface
    //
    global $file_manager_attachment;
    sqextractGlobalVar('file_manager_attachment');
    if (!isset($file_manager_attachment) || !$file_manager_attachment) {
...
function newMail () {
    ...
    if (isset($id)) {
        ...
        elseif ($reply_id) {
            ...
            // Modification for File Manager Plugin:
            // when sending attachments, the body
            // has already been retrieved, and possibly
            // altered by the user, so we want to get
            // it from $_POST
            //
            global $file_manager_attachment;
            if (!isset($file_manager_attachment) || !$file_manager_attachment) {
