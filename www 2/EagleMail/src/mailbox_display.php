 ...
 *
 * $Id: mailbox_display.php,v 1.321.2.20 2004/04/19 16:21:53 jangliss Exp $
 */
...
function showMessagesForMailbox($imapConnection, $mailbox, $num_msgs,
    ...
    echo '<tr><td HEIGHT="5" class="blank_bar"></td></tr>'; //top toggle bar
    echo '<tr><td class="blank_bar">';

   mail_message_listing_beginning($imapConnection, $mailbox, $sort,
                                $msg_cnt_str, $paginator_str, $start_msg);
    echo '</td></tr>';
    echo '<tr><td>';
    echo '    <table width="100%" cellpadding="1" cellspacing="0" align="center"'.' border="0" class="mail_display_header">';
    echo '     <tr><td>';
    echo '       <table width="100%" cellpadding="1" cellspacing="0" align="center" border="0" >'; 
    echo '<tr><td >'; //Main message header
...
//             . "&amp;startMessage=$start_msg";
        ...
        . html_tag( 'table' ,
            html_tag( 'tr',
                html_tag( 'td' ,
                    ...
                    , '', '', 'border="0" width="100%" cellpadding="0" cellspacing="0" class="blank_bar"' )
                , 'left', '', '' )
            , '', $color[0] )
	    , '', '', 'border="0" width="100%" cellpadding="0" cellspacing="0" class="blank_bar"' );

	/* line between header and button area */
      echo '<tr><td HEIGHT="5" class="blank_bar"></td></tr>'; //line bellow toggle 

        echo '<tr><td>';
        echo html_tag( 'tr' ) . "\n"
        . html_tag( 'td' ,'' , 'left', '', '' )
         . html_tag( 'table' ,'' , '', $color[4], ' border="0" width="100%" cellpadding="0" class="blank_bar" cellspacing="0"' )
	  . '<tr><td>'
           . html_tag( 'table' ,'' , '', $color[0], 'border="0" width="100%" cellpadding="0" class="message_toggle_bar" cellspacing="0"' )
            . html_tag( 'tr' )
            . html_tag( 'td', '', 'left', '', 'valign="middle" nowrap' );
		   getMbxList($imapConnection); 
            echo getButton('SUBMIT', 'moveButton', "    "._("Move"), 'move_button', 'class="move_button"'). " \n";   
            echo getButton('SUBMIT', 'attache', "    "._("Forward"), 'forward_button','class="forward_button"'). "\n"; 
  ...
  echo "      </small></td>\n"
	...
	echo getButton('SUBMIT', 'markFlagged', "    "._("Flag"), 'flag_button', 'class="flag_button"') . " \n";
    echo getButton('SUBMIT', 'markUnflagged', "    "._("Unflag"), 'unflag_button', 'class="unflag_button"') . " \n";
	echo getButton('SUBMIT', 'markRead', "    "._("Read"), 'read_button', 'class="read_button"') . " \n";
    echo getButton('SUBMIT', 'markUnread', "    "._("Unread"), 'unread_button', 'class="unread_button"') . " \n";
    echo getButton('SUBMIT', 'delete', "    "._("Delete"), 'delete_button', 'class="delete_button"') ."&nbsp;\n";
...
function mail_message_listing_end($num_msgs, $paginator_str, $msg_cnt_str, $color) {
  if ($num_msgs) {
    ...
    echo html_tag( 'table',
        ...
        , '', $color[9], 'width="100%" border="0" cellpadding="0"  cellspacing="0" ' );
...
function printHeader($mailbox, $sort, $color, $showsort=true) {
    global $index_order;
    echo html_tag( 'tr' ,'' , 'center', $color[5] ); //Main table header for mail messages
...
function get_paginator_str($box, $start_msg, $end_msg, $num_msgs,
    ...
    if (($next_grp <= $num_msgs) && ($prev_grp >= 0)) {
        $prv_str = get_paginator_link($box, $prev_grp, $use, _("<img src=\"../images/back.png\" border=\"0\" align=\"absmiddle\" alt=\"Previous\">"));
        $nxt_str = get_paginator_link($box, $next_grp, $use, _("<img src=\"../images/forward_grey.png\" border=\"0\" align=\"absmiddle\" alt=\"Next\">"));
    } else if (($next_grp > $num_msgs) && ($prev_grp >= 0)) {
        $prv_str = get_paginator_link($box, $prev_grp, $use, _("<img src=\"../images/back.png\" border=\"0\" align=\"absmiddle\" alt=\"Previous\">"));
        $nxt_str = "<FONT COLOR=\"$color[9]\">"._("<img src=\"../images/forward_grey.png\" border=\"0\" align=\"absmiddle\" alt=\"Next\">")."</FONT>\n";
    } else if (($next_grp <= $num_msgs) && ($prev_grp < 0)) {
        $prv_str = "<FONT COLOR=\"$color[9]\">"._("<img src=\"../images/back_grey.png\" border=\"0\" align=\"absmiddle\" alt=\"Previous\">") . '</FONT>';
        $nxt_str = get_paginator_link($box, $next_grp, $use, _("<img src=\"../images/forward.png\" border=\"0\" align=\"absmiddle\" alt=\"Next\">"));
    }
...
function getButton($type, $name, $value, $class, $background_img) {
    return '<INPUT TYPE="'.$type.'" NAME="'.$name.'" VALUE="'.$value . '" CLASS="'.$class.'" Background-image:"'.$background_img. '" >';
}
