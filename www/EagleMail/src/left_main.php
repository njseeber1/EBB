...
function formatMailboxName($imapConnection, $box_array) {
    ...
    if ($special_color) {
        $line .= "<font color=\"$color[8]\">";
    }
     if ( $mailbox == '' ) {
    		$line .= '<br><div id="left_inbox_button" title="'. $mailbox .'">'._("INBOX");
    //    $line .= _("&nbsp;&nbsp;INBOX");
    } elseif ( $real_box == $draft_folder){
    		$line .= '<br><div id="left_drafts_button" title="'. $mailbox .'">'._("Drafts").'</div>';
       // $line .= _("&nbsp;&nbsp;Drafts");
    } elseif ( $real_box == $sent_folder){
    		$line .= '<br><div id="left_sent_button" title="'. $mailbox .'">'._("Sent").'</div>';
      //  $line .= _("&nbsp;&nbsp;Sent");
    } elseif ( $real_box == $trash_folder){
    		$line .= '<br><div id="left_trash_button" title="'. $mailbox .'">'._("Trash");
      //  $line .= _("&nbsp;&nbsp;Trash");
    } else {
    		$line .= '<br><div id="left_inbox_button" title="'. $mailbox .'">';
...
function create_unseen_string($boxName, $boxArray, $imapConnection, $unseen_type) {
    ...
    if (($unseen_type == 1) && ($boxUnseenCount > 0)) {
        ...
        $result[0] = "<font color=\"$color[11]\">$result[0]</font></div>";
    }
...
function ListAdvancedBoxes ($boxes, $mbx, $j='ID.0000' ) {
	...
	if (!$boxes->is_root) {
	    if ($use_folder_images) {
	      if ($boxes->is_inbox) {
		$folder_img = '../images/inbox_left.png';
	      } else if ($boxes->is_sent) {
		$folder_img = '../images/sent_left.png';
	      } else if ($boxes->is_trash) {
		$folder_img = '../images/delete_left.png';
	      } else if ($boxes->is_draft) {
		$folder_img = '../images/drafts_left.png';
	      } else $folder_img = '../images/folder_left.png';
...
echo "\n<body bgcolor=\"$color[3]\" text=\"$color[8]\" link=\"$color[8]\" vlink=\"$color[8]\" alink=\"$color[8]\" background=\"../images/left_back.gif\">\n";
...
echo "\n\n" . html_tag( 'table', '', 'left', '', 'border="0" cellspacing="0" cellpadding="0" width="99%" height="99%"' ).
   html_tag( 'tr' ) .
   html_tag( 'td valign="top"', '', 'left' ) .
   '<left><div id="left_logo"><img src="'.$org_logo.'" ></div>'. "\n\n"; //<br><font size="4"><b>'. _("NUTSMAIL") . "</b><br /></font>\n\n";
...
echo '<table width="100%" class="left_mailbox"><tr><td valign="top" class="left_mailbox_top">';
echo 'Mailboxes';
echo '</td></tr>';
echo '<tr><td>';
...
echo '<br><br><tr><td class="left_mailbox_top">';
echo 'Calendar';
echo '</td></tr>';
...
//echo '<table height="50%" ><tr><td valign="middle">';
