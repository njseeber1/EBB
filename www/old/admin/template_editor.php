<?php
	...
	if ($action == "updateListingsFormElement")
	{
		...
		$sql = "UPDATE " . $config[table_prefix] . "listingsFormElements SET field_type = $sql_field_type, ";
	...
	if ($action == "DeleteUserFormElement")
	{
		...
		$sql = "DELETE FROM " . $config[table_prefix] . "listingsFormElements where id = $id";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false)
		{
			log_error($sql);
	...
	?>
		...
		$sql = "SELECT *, ID as id FROM " . $config[table_prefix] . "listingsFormElements ORDER BY rank, field_name";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false)
		{
			log_error($sql);
