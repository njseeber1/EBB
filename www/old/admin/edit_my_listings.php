<?php
	...
	if ($delete != "")
	{
		...
		// delete a listing
		$sql = "DELETE FROM " . $config[table_prefix] . "listingsDB WHERE ((ID = $sql_delete) AND (user_ID = $userID))";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false) log_error($sql);

		// delete all the elements associated with a listing
		$sql = "DELETE FROM " . $config[table_prefix] . "listingsDBElements WHERE ((listing_id = $sql_delete) AND (user_id = $userID))";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false)
		{
			log_error($sql);
		...
		$sql = "SELECT file_name, thumb_file_name FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = $sql_delete) AND (user_id = $userID))";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false)
		{
			log_error($sql);
		...
		$sql = "DELETE FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = $sql_delete) AND (user_id = $userID))";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false) 
		{
			log_error($sql);
	...
	if ($action == "update_listing")
	{
		...
		else
		{
			...
			if ($pass_the_form == "Yes")
			{
				...
				$sql = "UPDATE " . $config[table_prefix] . "listingsDB SET title = $sql_title, ";
	...
	if ($edit != "")
	{
		...
		<table border="<?php echo $style[form_border] ?>" cellspacing="<?php echo $style[form_cellspacing] ?>" cellpadding="<?php echo $style[form_cellpadding] ?>" width="<?php echo $style[admin_table_width] ?>" class="form_main">
			...
			<tr>
				<td width="<?php echo $style[image_column_width] ?>" valign="top" align="center" class="row_main">
					...
					<?php
					$sql = "SELECT caption, file_name, thumb_file_name FROM " . $config[table_prefix] . "listingsImages WHERE ((listing_id = '$edit') AND (user_id = '$userID'))";
					$recordSet = $conn->Execute($sql);
						if ($recordSet === false) log_error($sql);
		...
		$sql = "SELECT f.field_name, db.field_value, f.field_type, f.field_caption, f.default_text, f.field_elements, f.required FROM " . $config[table_prefix] . "listingsFormElements f left join " . $config[table_prefix] . "listingsDBElements db on db.field_name = f.field_name and db.listing_id = '$edit' AND db.user_id = '$userID' ORDER BY rank";
	...
	if ($edit == "")
	{
		...
		$sql = "SELECT ID, title, notes, expiration, active FROM " . $config[table_prefix] . "listingsDB WHERE user_ID = $userID ORDER BY ID";
		$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false) log_error($sql);
