...
if ($config[allow_user_signup] == "yes")
	...
	elseif ($user_email == "")
		...
		if ($pass_the_form == "Yes")
			...
			$sql = "INSERT INTO " . $config[table_prefix] . "UserDB (user_name, user_password, emailAddress, creation_date, last_modified, active) VALUES ($sql_user_name, $random_number, $sql_user_email, ".$conn->DBDate(time()).",".$conn->DBTimeStamp(time()).", $sql_set_active)";
			$recordSet = $conn->Execute($sql);
				if ($recordSet === false) log_error($sql);
			...
			$sql_edit_isAgent =  make_db_safe($config[user_default_agent]);
