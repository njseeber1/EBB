...
if ($active != "yes") 
	{ // make sure the person is active
		include("$config[template_path]/user_top.html");
		echo "<p><br>$lang[admin_new_user_moderated]</p>";
		die('<!-- login failed -->');
		include("$config[template_path]/user_top.html");
	}
else {
	...
	function renderFormElement($field_type, $field_name, $field_caption, $default_text, $field_elements, $required)
		...
		switch ($field_type)
		{
			...
			default: // the default handler -- for errors, mostly
				echo "<td align=\"right\" class=\"row_main\">no handler yet - $field_type</td>";
	...
	function renderExistingFormElement($field_type, $field_name, $field_value, $field_caption, $default_text, $required, $field_elements)
	{
		...
		switch ($field_type)
		{
			...
			case "text": // handles text input boxes
				...
				echo "</b></td>";
				echo "<td align=\"left\" class=\"row_main\"><input type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\"";
				if ($field_type == 'date')
				{
	...
	function handleUpload($type,$edit,$owner)
	{
		...
		if (is_uploaded_file($HTTP_POST_FILES['userfile']['tmp_name']))
		{
			...
			if ($pass_the_upload == "true")
			{
				...
				if ($check_width == "")
				{
					echo "<p>$realname $lang[upload_success].</p>";
				}
				else
				{
					echo "<p>$check_width</p>";
				}
