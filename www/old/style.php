<?php
	...
	function renderUserInfoOnListingsPage($listingID)
	{
		if ($listingID != "")
		{
			...
			if ($listing_user_ID != "")
			{
				$sql = "SELECT " . $config[table_prefix] . "UserDBElements.field_value, " . $config[table_prefix] . "userFormElements.field_type, " . $config[table_prefix] . "userFormElements.field_caption FROM " . $config[table_prefix] . "UserDBElements, " . $config[table_prefix] . "userFormElements WHERE ((" . $config[table_prefix] . "UserDBElements.user_id = $listing_user_ID) AND (" . $config[table_prefix] . "UserDBElements.field_name = " . $config[table_prefix] . "userFormElements.field_name)) ORDER BY " . $config[table_prefix] . "userFormElements.rank ASC";
				$recordSet = $conn->Execute($sql);
				if ($recordSet === false)
				{
					log_error($sql);
	...
	function renderFeaturedListingsHorizontal($num_of_listings)
	{
		...
		function renderFeaturedListingsVerticalInfo($num_of_listings)
	{
		echo "<table><tr>";
		// shows the images connected to a given image
			...
			while (!$recordSet->EOF)
				{
					...
					while (!$recordSet2->EOF)
					{
						...
						echo "<a href=\"listingview.php?listingID=$ID\"> ";
						echo "<b>$Title</b><br>";
						echo "<img src=\"$config[listings_view_images_path]/$thumb_file_name\" height=\"$displayheight\" width=\"$displaywidth\" alt=\"$lang[click_to_learn_more]\"></a><br> ";
							while (!$recordSet3->EOF) {
								...
								//Print("<b><font size=\"2\">Beds: ".$beds." Baths: ".$baths."</font></b><br>"); //original method of displaying beds and baths
								Print("<b><font size=\"2\">".$beds." Beds/".$baths." Baths</font></b><br>");
								Print("<b>".money_formats($priceformatted)."</b><br>");
								Print("<br>");
		...
		echo "</tr></table>";
	} // end function renderFeaturedListingsVerticalInfo

	function renderFeaturedListingsHorizontalInfo($num_of_listings)
	{
		// shows the images connected to a given image
		...
		if ($returned_num_listings > 0)
		{
			...
			while (!$recordSet->EOF)
			{
				...
				if ($count % $numcols ==0)
				{
					print "<td align=\"center\"><a href=\"listingview.php?listingID=$ID\"><b>$Title</b><br>";
					print "<div align=\"center\"><img src=\"$config[listings_view_images_path]/$thumb_file_name\" height=\"$displayheight\" width=\"$displaywidth\" alt=\"$lang[click_to_learn_more]\" border=1></a><br>";
					Print("<b><font size=\"2\">".$beds." Beds/".$baths." Baths</font></b><br>");
					Print("<b>".money_formats($priceformatted)."</b><br>");
					echo "</div></td></tr><tr>";
				}
	...
	} // end function renderFeaturedListingsHorizontalInfo

		function searchbox_keyword ($caption, $field) 
		{ 
		global $conn, $config; 
		$r = $conn->getOne("select f.field_elements from " . $config[table_prefix] . "listingsFormElements f where field_name = '$field'");
		echo "<tr><td align=\"right\"><b>$caption</b></td>"; 
		echo "<td align=\"left\"><input type=\"text\" name=\"{$field}\"></td></tr><br>"; 
		}
